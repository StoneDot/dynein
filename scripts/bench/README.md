...
| `watchdog.sh` | Off-instance monitor **with abort authority**: CloudWatch stall detection (zero-consumption on any table + fractional-target on expensive ones, F1), spend ceiling, heartbeat age, external cell deadlines, phase-aware progress-frozen wedge detection (F2). Abort = tables → SSM salvage → stop instances → report. Start it right after every `--execute` | dev machine |
