...
- **v1 limitations** (recorded deliberately):
  - Provisioned tables only. Warm throughput is readable now (used for the initial target, §4.6) and could serve as the on-demand headroom reference, but "warm − production" as a boost target is a semantic extension that needs its own design pass (warm throughput grows with usage, unlike a provisioned entitlement) — deliberately not done yet
