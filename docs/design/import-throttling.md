...
- AIMD verification (10-WCU table, 2000 items, sustained throttling): the effective target halved stepwise from the hardcoded 100,000 down to ~98 while throttling persisted, and the import completed with exit 0. This run motivated the known-information initial target: starting from the absurd 100,000 ceiling took ~13 halvings (~30 s) to reach realistic levels. The `--max-wcu` CLI option (roadmap 4) still matters for setting a sane ceiling
